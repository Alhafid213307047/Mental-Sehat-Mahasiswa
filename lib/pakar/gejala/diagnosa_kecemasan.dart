...
class DiagnosaKecemasan extends StatefulWidget {
  const DiagnosaKecemasan({Key? key}) : super(key: key);

  @override
  _DiagnosaKecemasanState createState() => _DiagnosaKecemasanState();
}

class _DiagnosaKecemasanState extends State<DiagnosaKecemasan> {
  ...
  Future<void> _showTambahGejalaDialog(BuildContext context) async {
    try {
      ...
      _kodeController.text = 'K${_nextKodeSuffix.toString().padLeft(3, '0')}';
    } catch (e) {
      print('Error: $e');
    }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Apakah Anda yakin ingin menghapus gejala "$kodeGejala"?',
            style: TextStyle(
              fontFamily: 'Poppins',
            ),
          ),
